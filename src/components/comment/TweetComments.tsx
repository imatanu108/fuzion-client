...
import useTweetComments from '@/hooks/comment/useTweetComments';
import CommentCard from './CommentCard';

const TweetComments: React.FC = () => {
    const { id } = useParams();
    ...
    const tweetComments = useTweetComments(String(id), refreshTrigger);

    useEffect(() => {
        setCommentsCount(tweetComments.length)
    }, [tweetComments, refreshTrigger, id])
                    ...
                    {tweetComments && tweetComments.length ? (
                        tweetComments.length > 1
                            ? showAllComments
                                ...
                                : (
                                    <CommentCard
                                        key={tweetComments[0]._id}
                                        comment={tweetComments[0]}
                                        reduceCommentCount={reduceCommentCount}
                                    />
                                )
                            : (
